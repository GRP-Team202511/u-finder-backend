"""
Router tests for POST /api/admin/users/{userId}/unblock (Unblock User)
...
UNBLOCK_URL = "/api/admin/users/{userId}/unblock"
...
def _make_target_user(user_id: int = 5, *, user_type: int = UserType.STUDENT, is_blocked: bool = True):
    """Return a mock Account representing the user to be unblocked."""
    acc = MagicMock()
    acc.user_id = user_id
...
# ── POST /api/admin/users/{userId}/unblock Tests ──────────────────────────────

class TestUnblockUser:

    async def test_unblock_user_success(self, client, mock_db):
        """Admin unblocks a blocked user -> 200 with success result."""
        ...
        response = await client.post(
            UNBLOCK_URL.format(userId=5), headers=_admin_auth_header()
        )
    ...
    async def test_unblock_already_active_user_idempotent(self, client, mock_db):
        """Unblocking an already-active user still returns 200."""
        admin = _make_admin_account()
        target = _make_target_user(user_id=5, is_blocked=False)
    ...
    async def test_unblock_admin_account_forbidden(self, client, mock_db):
        """Attempting to unblock another admin -> 403."""
        admin = _make_admin_account()
        ...
        response = await client.post(
            UNBLOCK_URL.format(userId=10), headers=_admin_auth_header()
        )
    ...
    async def test_unblock_user_not_found(self, client, mock_db):
        """Unblocking a non-existent user -> 404."""
        admin = _make_admin_account()
        ...
        response = await client.post(
            UNBLOCK_URL.format(userId=999), headers=_admin_auth_header()
        )

        assert response.status_code == 404
        assert_message_response(response.json(), "User not found")

    async def test_unblock_user_missing_auth_header(self, client):
        """Missing Authorization header -> 422."""
        response = await client.post(UNBLOCK_URL.format(userId=5))
        assert response.status_code == 422

    async def test_unblock_user_invalid_token(self, client, mock_db):
        """Invalid Bearer token -> 401."""
        response = await client.post(
            UNBLOCK_URL.format(userId=5),
    ...
    async def test_unblock_user_non_admin(self, client, mock_db):
    ...
    async def test_unblock_user_response_fields(self, client, mock_db):
